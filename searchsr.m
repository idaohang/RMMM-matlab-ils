function [Zhat,numExpanded] = searchsr(R,y,p)
%
...
%Compute min diag for all values of k
minDiag = zeros(1,n);
for i = 1:n
    minDiag(i) = min(abs(diag(R(1:i,1:i))));
end
while 1
    ...
    if newprsd < beta
        if k ~= 1 % move to level k-1
            ...
            %Check the packing radius conditions from level 1->n and move
            %back up the tree as far as possible
            tempK = 2;
            while( tempK<=n+1 && norm(y(1:tempK-1) - R(1:tempK-1,tempK:n)*z(tempK:n) - R(1:tempK-1,1:tempK-1)*z(1:tempK-1)) <= (0.5*minDiag(tempK-1)) )
                tempK = tempK + 1;
            end
            if(tempK == n+1)
                return;
            end
            %Move up to level tempK
            k = tempK;
            z(k) = z(k) + d(k);
            gamma = R(k,k)*(c(k)-z(k));
            if d(k) > 0
