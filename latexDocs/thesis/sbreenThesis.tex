...
\SetAbstractEnText{
This thesis is concerned with integer least squares (ILS) problems. One often used approach for ILS problems is the discrete search method which typically involves two stages, the reduction and the search.\\
\indent The box-constrained integer least squares problem (BILS) arises in MIMO wireless
communications applications. In order to make the search algorithm
more efficient, the columns of the the input matrix in the BILS problem have to
be reordered. There are currently two algorithms for column reordering that provide the best known results. 
Both use all available information, but they were derived respectively from geometric and algebraic points of view
and look different. In this thesis we modify one to make it more computationally efficient and easier to comprehend.
...
and is still easy to understand. This thesis will also consider the reduction of ordinary integer least squares (OILS) problems. For OILS problems, another new reduction algorithm is derived which is based on the new BILS reduction. This new reduction algorithm is shown to be more efficient than current reduction algorithms in some practical applications.\\
\indent We will also review some common algorithms for the search stage of the ILS problem. A new algorithm is proposed which is based on a combination of two previous algorithms. This new algorithm is shown to be more efficient and easier to implement than previous algorithms for some practical applications.
}
\AbstractEn%
...
We no longer have a closed-form representation for the optimal solution $x_{\scriptscriptstyle{ILS}}$ in this case, in fact, the
