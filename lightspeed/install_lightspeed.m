...
fprintf('Compiling lightspeed 2.4 mex files...\n');
...
elseif ismac
	...
	options.COMPFLAGS = '-fno-common -no-cpp-precomp -arch x86_64 -isysroot /Developer/SDKs/MacOSX10.5.sdk -mmacosx-version-min=10.5  -fexceptions -DMX_COMPAT_32';
...
if ispc
  ...
  flags = [flags ' -DUNDERSCORE_LAPACK_CALL'];
  if atleast76
    lapacklib = '-lmwlapack';
    blaslib = '-lmwblas';
  end
end
disp(['mex ' flags ' solve_triu.c "' lapacklib '" "' blaslib '"'])
...
if ispc
  ...
  mex gammaln.c util.obj -largeArrayDims -outdir @double
  ...
  mex gammaln.c util.o -lm -largeArrayDims -outdir @double
  ...
  if ismac
    % thanks to Nicholas Butko for these mac-specific lines
		...
		system([options.COMPILER ' ' options.COMPFLAGS ' ' options.OPTIMFLAGS ' -fPIC -c random.c; ' options.COMPILER ' -dynamiclib -Wl,-install_name,`pwd`/librandom.dylib -o librandom.dylib random.o'])
    ...
    system([options.COMPILER ' ' options.COMPFLAGS ' ' options.OPTIMFLAGS ' -fPIC -c random.c; ' options.COMPILER ' -shared -Wl,-E -Wl,-soname,`pwd`/librandom.so -o librandom.so random.o'])
