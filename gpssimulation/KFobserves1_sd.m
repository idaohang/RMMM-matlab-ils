function [visible,sd,error,gn] = KFobserves1_sd(x_rcvr1,x_rcvr2,dx_rcvr2e,iNs,sigma,t,t_s,min_elev,ephd,n_vis_sat_max,t_fault,failsat,bias_rate)
% DESCRIPTION
...
for i=1:n_vis_sat,
  ...
  %fail = failure1(i,t-t_s,t_fault-t_s,failsat,bias_rate); % failure in receiver (RAIM)%%%%%%%%%%%%%%%%
  fail=0;%%%%%%%%%%%%%%%%%
  r_rcvr2(i) = sqrt(sum((svv(i,:)-x_rcvr2).^2));
  gnoise=noiseg(0,sigma);
  
  sd(i) = (r_rcvr1(i)-r_rcvr2(i))/L1_W - iNs(i)+(gnoise+fail)/L1_W;
  error(i) = fail;
  gn(i)=gnoise;
