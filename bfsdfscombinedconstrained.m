function [estimate, numExpanded] = bfsdfscombinedconstrained(R,y,lower,upper,k,maxUnsolved,maxBabai,maxNodes)
%
...
%    search
    ...
    estimate = zeros(1,n);
    if(k >= n-1)
        [estimate, numExpanded] = searchConstrained(R,y,lower,upper,1);
        return;
    end
        ...
        if(nextChildCost(curNode) < rad) %this line can be removed... unnecessary for a pure best  first search!
           if(treeLevel(curNode) > 2) %if the next best child is not a leaf, calculate its attributes and push it to PQb
                ...
                if( i == k)
                    ...
                    if(rad == inf)
                        z0 = constrainedBabai(R(1:k,1:k),y2,lower,upper);
                        if(unsolvedCount < maxUnsolved && norm(R(1:k,1:k)*z0 - y2)^2 > maxBabai)
                            ...
                            [subZ,subRad,subExpand] = searchExtraConstrained(R(1:k,1:k),y2,rad,cost(curChildIdx),lower,upper);
                        end
                    else                  
                        [subZ,subRad,subExpand] = searchExtraConstrained(R(1:k,1:k),y2,rad,cost(curChildIdx),lower,upper);
                    end
                    ...
                    if(nextChildCost(pq_top(PQb)) >= rad)
                        ...
                        for i=1:unsolvedCount
                            [subZ,subRad,subExpand] = searchExtraConstrained(R(1:k,1:k),unsolvedY(:,i),rad,cost(unsolvedNode(i)),lower,upper);
                            subNumExpanded = subNumExpanded+subExpand;
