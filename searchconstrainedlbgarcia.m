function [Zhat,numExpanded] = searchconstrainedlbgarcia(Q,R,y,lower,upper,p)
%
...
inverseNorms = getInverseColumnNorms(Q,R);
...
while 1
    ...
    if(newprsd + sigmas(k) <= beta)
        if(k ~= 1 && k~= n)  
            ...
            lb = max(abs(round(f(1:k-1,k-1)) - f(1:k-1,k-1))./inverseNorms(1:k-1,k-1));
        else
            if(k == n)
